...
/*
 * PrintDot
 * emit concept lattice in dot(1) format
 */
...

{
	...
	/* print all concepts - each nodes is annotated with the 
	...
	for (i=0;i<lattice->conc;i++) {
		...
		atrsetmin = &concept->atr; /* NEW attributes */
                objs  = SetSize(&concept->objects);
                attrs = SetSize(&concept->attributes);
                
                area     = (double) objs * (double) attrs;
