...
#if 0
#define CONCEPTS_FORMAT "%-%O%n"
#else
#define CONCEPTS_FORMAT "   objects[%i]:%t%o%nattributes[%i]:%t%a%n"
#endif
...
static char *usage = 
	"Usage: %s [-o file] [-f format] -c | -g | -G | -a | -s [file] \n" ;
	...
	char **argv;		/* argument vekctor */
{
	...
	infile = (char*) NULL;
	graph = Graph = size = arrows = concepts = 0;
	...
	while ((c=getopt(argc,argv,"o:f:ascgGh")) != -1) {
		switch (c) {
		case 'a':
			arrows = 1;
	...
	if (errflag 
	    || (arrows+size+graph+concepts+Graph != 1)) {
